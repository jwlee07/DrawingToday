...
class VideoPlayerViewController: BaseViewController {
    ...
    lazy var videoPlayerView = VideoPlayerView(viewWidth: (deviceWidth * widthRatio) - (padding * 2),
                                               viewHeight: (deviceHeight * heightRatio) / 2)
    // MARK: - LifeCycle
...
extension VideoPlayerViewController: BaseViewSettingProtocol {
    ...
    func setLayouts() {
        videoPlayerView.snp.makeConstraints {
            ...
            $0.height.equalTo((deviceHeight * heightRatio) / 2)
        }
    }
    func createViews() {
        setAddSubViews()
